"""Few-shot banking intent classification with a local Ollama model.

This experiment uses five static training examples per intent in the prompt.
...
def main():
    """Load dataset, run few-shot inference with the Ollama model, and evaluate."""
    dataset = IntentsDataset()

    prompt = FewShotPrompt(dataset.X_train, dataset.y_train, n_per_class=5)
    ...
    experiment = f'llm-fewshot-{evaluation.model_slug(client.model_name)}'
